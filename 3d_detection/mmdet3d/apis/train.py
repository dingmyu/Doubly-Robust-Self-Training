...
from mmdet3d.datasets import build_dataloader as build_self_labeling_dataloader
...
def train_self_labeling_detector(model,
                   dataset,
                   cfg,
                   distributed=False,
                   validate=False,
    ...
    runner_type = 'EpochBasedRunner' if 'runner' not in cfg else cfg.runner[
        'type']
    data_loaders = [
        build_self_labeling_dataloader(
            ds,
            cfg.data.samples_per_gpu,
            cfg.data.workers_per_gpu,
            # `num_gpus` will be ignored if distributed
...
def train_model(model,
    ...
    if cfg.model.type in ['EncoderDecoder3D']:
        train_segmentor(
            ...
            timestamp=timestamp,
            meta=meta)
    elif cfg.model.type in ['CenterPointSelfLabeler','CenterPointBiDLabeler']:
        train_self_labeling_detector(
            model,
            dataset,
            cfg,
            distributed=distributed,
