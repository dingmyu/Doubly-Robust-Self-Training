...
parser.add_argument('--pseudomode', type=int, default=0,
                    help='')
...
def main():
    ...
    # for index, item in enumerate(loader_train):
    #     print(item)
    #     break
...
def train_one_epoch(
    ...
    for batch_idx, (input, target, pseudo) in enumerate(loader):
        ...
        if not args.prefetcher:
            input, target, pseudo = input.cuda(), target.cuda(), pseudo.cuda()
            if mixup_fn is not None:
                input, target = mixup_fn(input, target)
        if args.channels_last:
            input = input.contiguous(memory_format=torch.channels_last)

        pseudo_only = (args.pseudomode == 0)
        mixture = (args.pseudomode == 1)
        doubly_robust = (args.pseudomode == 2)
        ...
        with amp_autocast():
            ...
            if pseudo_only:
                ...
                gt_labels = (pseudo != -1)
                pseudo_labels = (pseudo == -1)
                gt_idx = np.where(np.array(gt_labels.cpu()))[0]
                pseudo_idx = np.where(np.array(pseudo_labels.cpu()))[0]
                gt_outs = output[gt_idx]
                pseudo_outs = output[pseudo_idx]
                gt_labels = pseudo[gt_idx]
                pseudo_labels = target[pseudo_idx]
                gt_loss = sum(loss_fn(gt_outs, gt_labels)) if len(gt_labels) else 0
                pseudo_loss = sum(loss_fn(pseudo_outs, pseudo_labels)) if len(pseudo_labels) else 0
                loss = (gt_loss + pseudo_loss) / len(target)
            elif doubly_robust:
                gt_labels = (pseudo != -1)
                pseudo_labels = (pseudo == -1)
                gt_idx = np.where(np.array(gt_labels.cpu()))[0]
                ...
                gt_labels = pseudo[gt_idx]
                pseudo_labels = target[pseudo_idx]
                gt_pseudo_labels = target[gt_idx]
                gt_loss = sum(loss_fn(gt_outs, gt_labels)) if len(gt_labels) else 0
                pseudo_loss = sum(loss_fn(pseudo_outs, pseudo_labels)) if len(pseudo_labels) else 0
                gt_pseudo_loss = sum(loss_fn(gt_outs, gt_pseudo_labels)) if len(gt_pseudo_labels) else 0
                M = float(len(pseudo_idx))
                N = float(len(gt_idx))
                if N > 0:
                    loss = (pseudo_loss + gt_pseudo_loss)/(M + N) + (gt_loss - gt_pseudo_loss) / N * (epoch + (batch_idx + 1) / len(loader)) / args.epochs
                else:
                    loss = pseudo_loss / M
